...
for threshold in thresholds:
    df['group'] = np.where(df['utilisation_ratio'] > threshold, 'Group1', 'Group2')

    model_summ_row = [threshold]
    ...
    for group in df['group'].unique():
        ...
        df.loc[df_train.index, 'fitted_values_' + str(threshold)] = res.fittedvalues
        df.loc[df_test.index, 'fitted_values_' + str(threshold)] = res.predict(df_test)
        df_group['fitted_values'] = df['fitted_values_' + str(threshold)]
        ...
        model_summ_row.extend([r2, aic_val, max_pval])

    model_summary.append(model_summ_row)

# Create a new dataframe for the model summary
df_model_summary = pd.DataFrame(model_summary, columns=['Threshold', 'R2_1', 'AIC_1', 'Max_pval_1', 'R2_2', 'AIC_2', 'Max_pval_2'])
